...
namespace Mail_Box
{
    public partial class Inbox_Trash : Form
    {
        int type = 3;
        public Inbox_Trash()
        {
        ...
        private void getInboxTrash()
        {
            SqlConnection conn = new SqlConnection(Global.connectionString);
            try
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("showTrashInbox", conn);
                ...
                if (reader.HasRows)
                {
                    ...
                    dt.Load(reader);
                    InboxTrash_DGV.DataSource = dt;
                }

                InboxTrash_DGV.AllowUserToAddRows = false;
                InboxTrash_DGV.Columns["Date_Time"].Width = 120;

                DataGridViewButtonColumn btn = new DataGridViewButtonColumn();
                InboxTrash_DGV.Columns.Add(btn);
                ...
                InboxTrash_DGV.Columns.Add(delete);
                delete.Text = "Delete";
                ...
                InboxTrash_DGV.Columns.Add(undelete);
                undelete.Text = "unDelete";
                undelete.HeaderText = "unDelete";
                undelete.Name = "unDelete";
                undelete.UseColumnTextForButtonValue = true;

                InboxTrash_DGV.Width = InboxTrash_DGV.Columns.Cast<DataGridViewColumn>().Sum(x => x.Width)
                        + (InboxTrash_DGV.RowHeadersVisible ? InboxTrash_DGV.RowHeadersWidth : 0) - 95;
                InboxTrash_DGV.Columns[0].Visible = false;
            }
            catch (SqlException ex) { Console.WriteLine(ex.Message); }
            catch (NullReferenceException ex)
            {
                Console.WriteLine(ex.Message);
                MessageBox.Show("your inbox trash is empty!!!", "Empty Inbox Trash");
                this.Hide();
                Main m = new Main();
                try { m.ShowDialog(); }
        ...
        private void InboxTrash_DGV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                try
                {
                    string mid = InboxTrash_DGV.Rows[e.RowIndex].Cells[3].Value.ToString();
                    this.Hide();
            ...
            if (e.ColumnIndex == 6)
            {
                try
                {
                    string message = InboxTrash_DGV.Rows[e.RowIndex].Cells[6].Value.ToString();
            ...
            if (e.ColumnIndex == 2)
            {
                try
                {
                    ...
                    {
                        conn.Open();
                        SqlCommand cmd = new SqlCommand("undeleteInboxMessage", conn);
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void deleteMessage(string mid)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("deleteInboxMessageCompletly", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@mid", mid);
                cmd.ExecuteNonQuery();
            }
        ...
        private void search_B_Click(object sender, EventArgs e)
        {
            ...
            else if (searchBy_CB.Text != "Date" && searchBy_CB.Text != "From..To Date")
            {
                ...
                else
                {
                    ...
                    {
                        conn.Open();
                        SqlCommand cmd = new SqlCommand("searchInboxTrash", conn);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@username", Global.loginUser);
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        conn.Open();
                        SqlCommand cmd = new SqlCommand("searchInboxTrashbyDate", conn);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@username", Global.loginUser);
